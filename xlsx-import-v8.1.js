(function (global) {
  ...
  function sheetMatrix(sheet) {
    ...
    for (let r = 0; r < sheet.rowCount; r += 1) {
      const row = [];
      for (let c = 0; c < sheet.colCount; c += 1) row.push(normalizeText(sheet.grid[r]?.[c] ?? ""));
      rows.push(row);
    }
    return rows;
  }
  ...
  function detectHeaderRow(sheet) {
    ...
    matrix.slice(0, Math.min(20, matrix.length)).forEach((row, index) => {
      const nonempty = row.filter(Boolean).length;
      const keywordHits = row.filter((value) => keywords.test(value)).length;
      const score = nonempty + keywordHits * 3;
      if (score > best.score) best = { row: index, score };
    });
    return best.row;
  }

  function genericTable(sheet, headerRow = detectHeaderRow(sheet)) {
    const matrix = sheetMatrix(sheet);
    const rawHeaders = matrix[headerRow] || [];
    const headers = rawHeaders.map((value, index) => value || `Column ${index + 1}`);
    const rows = matrix.slice(headerRow + 1).map((values, rowOffset) => ({
      sourceRow: headerRow + rowOffset + 2,
      values,
      record: Object.fromEntries(headers.map((header, index) => [header, values[index] || ""])),
    })).filter((item) => item.values.some(Boolean));
    return { sheetName: sheet.name, headerRow: headerRow + 1, headers, rows };
  }

  function inferGenericDestination(table) {
    const text = `${table.headers.join(" ")} ${table.rows.slice(0, 12).flatMap((row) => row.values).join(" ")}`.toLowerCase();
    const score = { schedule:0, calendar:0, playbook:0, library:0, learners:0, plans:0, memory:0, tasks:0 };
    const add = (key, pattern, points=1) => { if (pattern.test(text)) score[key] += points; };
    add("schedule", /monday|tuesday|wednesday|thursday|friday|start time|end time|period|星期一|星期二|课表/, 4);
    add("calendar", /holiday|school closed|early dismissal|testing|assembly|field trip|calendar|date|放假|校历|日期/, 3);
    add("playbook", /activity|purpose|steps|procedure|teacher language|variation|活动|目的|步骤|玩法|教学语言/, 4);
    add("library", /resource|file|url|link|worksheet|slides|skill|topic|资源|链接|工作纸|课件/, 3);
    add("learners", /student|learner|grade|profile|学生|学习者|年级/, 3);
    add("plans", /lesson plan|objective|goal|next lesson|教学计划|目标|下一课/, 4);
    add("memory", /observation|what worked|challenge|next step|观察|有效|困难|下一步/, 4);
    add("tasks", /task|to-do|due date|priority|agenda|待办|截止|优先级/, 4);
    return Object.entries(score).sort((a,b)=>b[1]-a[1])[0][0] || "library";
  }
