...
function renderLearnerListV3C(){
 ...
 if(learnerTabV3C==="individuals"){
   ...
   target.innerHTML=list.map(x=>{const members=(x.memberIds||[]).map(learnerByIdV3C).filter(Boolean),plans=data.learnerPlans.filter(p=>p.planType==="group"&&p.targetId===x.id&&p.status!=="Completed").length;return`<article class="learner-card group-card"><div class="learner-card-heading"><div class="learner-avatar group-avatar">G</div><div><span class="eyebrow">Small group</span><h4>${escapeHTML(x.name)}</h4><p>${escapeHTML(x.focus||"No focus added")}</p></div><button class="icon-button" data-edit-group="${x.id}" aria-label="Edit ${escapeHTML(x.name)}">✎</button></div><div class="member-chip-row">${members.map(m=>`<span class="member-chip">${escapeHTML(m.name)}</span>`).join("")||'<span class="member-chip muted">No members selected</span>'}</div><div class="learner-card-body"><div><span>Cadence</span><p>${escapeHTML(x.cadence||"Not specified")}</p></div><div><span>Notes</span><p>${escapeHTML(x.notes||"No notes added yet.")}</p></div></div><div class="learner-card-footer"><span>${plans} active plan${plans===1?"":"s"}</span><div><button class="text-button" data-memory-for-group="${x.id}">Add memory</button><button class="text-button" data-plan-for-group="${x.id}">Create plan</button></div></div></article>`}).join("")||'<div class="empty-state">No small groups yet. Whole classes remain in Weekly Planner.</div>';
 }
}
function renderPlanListV3C(){
 ...
 target.innerHTML=list.map(x=>{const targetRecord=planTargetV3C(x),name=targetRecord?.name||x.targetName||"Unassigned";return`<article class="learner-plan-card"><div class="plan-status-row"><span class="plan-type-pill">${x.planType==="group"?"Small-Group Plan":"Individual Plan"}</span><span class="plan-status ${String(x.status).toLowerCase()}">${escapeHTML(x.status)}</span></div><h4>${escapeHTML(x.title)}</h4><p class="plan-target">${escapeHTML(name)}</p><dl><div><dt>Focus</dt><dd>${escapeHTML(x.focus||"Not specified")}</dd></div><div><dt>Goals</dt><dd>${escapeHTML(x.goals||"No goals added yet.")}</dd></div><div><dt>Next</dt><dd>${escapeHTML(dateLabelV3C(x.nextDate))}${x.cadence?` · ${escapeHTML(x.cadence)}`:""}</dd></div></dl><div class="tag-row">${String(x.tags||"").split(",").map(v=>v.trim()).filter(Boolean).map(t=>`<span class="tag">${escapeHTML(t)}</span>`).join("")}</div><div class="learner-card-footer"><span>${x.legacyLessonId?"Migrated session":"Local plan"}</span><div><button class="mini-button" data-memory-for-plan="${x.id}">Memory</button><button class="mini-button" data-schedule-plan="${x.id}">Schedule</button><button class="mini-button" data-edit-plan="${x.id}">Edit</button></div></div></article>`}).join("")||`<div class="empty-state">No ${planTabV3C==="group"?"small-group":"individual"} plans match these filters.</div>`;
}
